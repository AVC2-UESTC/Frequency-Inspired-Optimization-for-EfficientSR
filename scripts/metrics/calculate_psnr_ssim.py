...
def main(args):
    ...
    for i, img_path in enumerate(img_list_gt):
        ...
        h_old, w_old, _ = img_restored.shape
        print(img_restored.shape)
        img_gt = img_gt[:h_old, :w_old, ...]  # crop gt
        print(img_gt.shape)
        ...
        elif args.test_y_channel and img_gt.ndim == 2:
            img_restored = bgr2ycbcr(img_restored, y_only=True)
    ...
    print(args.gt)
    print(args.restored)
    print(f'Average: PSNR: {sum(psnr_all) / len(psnr_all):.2f} dB, SSIM: {sum(ssim_all) / len(ssim_all):.4f}')
